...
export const STORAGE_KEY_MODEL = 'kinber-preferred-model-v2';
...
// ---------------------------------------------------------------------------
// TEMPORARY SAFE STUB — replaces old Suna backend model API
// ---------------------------------------------------------------------------
...
export const useModelSelection = () => {
  ...
  // ✅ Fetch available models safely (no missing queryFn)
  const {
    data: modelsData,
    isLoading: isLoadingModels,
    refetch: refetchModels,
  } = useAvailableModels() as { data?: { models?: any[] }; isLoading: boolean; refetch: () => void };

  
    const subscriptionStatus: SubscriptionStatus = 'active';

  // You can optionally handle refetch manually if needed:
  useEffect(() => {
    if (!modelsData && !isLoadingModels) {
      refetchModels(); // ensures retry on mount if data not loaded
    }
  }, [modelsData, isLoadingModels, refetchModels]);
