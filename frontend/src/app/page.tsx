...
export default function MarketingHome() {
  ...
  // 🔐 Redirect authenticated users only
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#0b0b0c] text-white overflow-x-hidden">
      <header className="sticky top-0 z-50 bg-[#0b0b0c]/80 backdrop-blur border-b border-white/10">
        <Nav />
      </header>
      ...
      <footer className="border-t border-white/10 mt-24 py-10 text-center text-sm text-white/60">
        © {new Date().getFullYear()} Kinber. All rights reserved.
      </footer>
    </div>
  );
}
